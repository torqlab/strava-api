 ...
 * @internal
 */
const fetchTokenResponse = async (body: URLSearchParams): Promise<Response> => {
  try {
...
const exchangeToken = async (
  ...
  if (!config.clientId || !config.clientSecret) {
    throw createError('INVALID_CONFIG', 'Client ID and client secret are required');
  } else if (!config.redirectUri) {
    throw createError('INVALID_CONFIG', 'Redirect URI is required');
  } else if (!authorizationCode) {
    throw createError('INVALID_CODE', 'Authorization code is required');
  } else {
